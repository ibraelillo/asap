...
export const createBot = (
  ...
  const warn = (event: string, details?: Record<string, unknown>) => {
    if (details) {
      console.warn(`${now()} ${botPrefix} ${event}`, {
        ...details,
        ...state(),
      });
      return;
  ...
  const hasEnoughMargin = async () => {
    ...
    const totalRequired =
      cfg.base *
      (1 +
        cfg.steps.slice(0, maxSteps).reduce((sum, s) => sum + s.sizeMult, 0));
  ...
  const fillQueue = async (pos: Position) => {
    ...
    for (let i = 0; i < maxSteps; i++) {
      ...
      const dcaPrice = normalizePrice(avg * (1 + direction * dist), tickSize);
  ...
  const addTakeProfit = async (pos: Position) => {
    ...
    if (lastAvgEntry !== avg || lastSize !== size) {
      try {
        if (orders.get("tp")) {
          try {
            ...
            const { cancelledOrderIds } =
              await service.orders.cancelOrder(existingId);
        ...
        err("Failed to place take-profit order", error, {
          avgEntry: livePos.avgEntryPrice,
        });
      }
    } else {
  ...
  const positionChanged = async (pos: Position) => {
    try {
      ...
      if (!size || !isFinite(size)) {
        warn("Position update skipped due to invalid size", {
          incomingQty: pos.currentQty,
        });
        return;
      }
      ...
      if (activeDcaOrderId) {
        ...
        if (!stillOpen) {
          log(
            "Active DCA order disappeared from active orders, assuming filled",
            {
      ...
      err("Failed to handle position update", error, {
  ...
  const positionClosed = async (pos: Position) => {
    try {
      ...
      err("Failed to handle closed position", error, {
