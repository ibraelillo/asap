...
//
//
export const takeProfitPrice = (current: number, side: 'LONG'| 'SHORT', percent = 1, multiplier = 1) =>  {

    const c = normalizePrice(current, multiplier)

    const tp = normalizePrice(c * (side === 'LONG' ? (1 + percent / 100) : (1- percent/100)), multiplier)
...
export const securityOrder = (current: number, side: 'LONG'| 'SHORT', percent = 1, multiplier = 1) =>  {
    const c = normalizePrice(current, multiplier)

    const pow = getPow(multiplier)

    const so = normalizePrice(c * (side === 'LONG' ? (1 - percent / 100) : (1 + percent/100)), multiplier)
