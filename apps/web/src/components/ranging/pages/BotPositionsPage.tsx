...
export function BotPositionsPage() {
  ...
  if (!positions && isLoading) {
    return <div className="panel p-6 text-sm text-slate-300">Loading positions...</div>;
  }
  ...
  const symbol = details?.summary && "symbol" in details.summary
    ...
    <div className="space-y-6">
      <header className="panel p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-cyan-300/80">Bot Positions</p>
            <h1 className="mt-2 text-3xl font-semibold text-slate-100">{symbol}</h1>
            <p className="mt-2 text-sm text-slate-300/90">
              Persisted position ledger and exchange reconciliation view for this bot.
            </p>
          </div>
          <Link
      ...
      <section className="panel p-5">
        <SectionHeader title="Positions" description="Local position records, not just exchange snapshots." />
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {positions.map((position) => (
                <tr key={position.id} className="border-t border-white/5 text-slate-200">
                  <td className="py-3 pr-4 text-xs text-slate-300 mono">{position.id}</td>
                  ...
                  <td className="py-3 pr-4">{position.avgEntryPrice?.toLocaleString() ?? "-"}</td>
                  <td className="py-3 pr-4">{position.stopPrice?.toLocaleString() ?? "-"}</td>
                  <td className="py-3 pr-4 text-xs text-slate-300">{formatDateTime(position.openedAtMs)}</td>
                  <td className="py-3 text-xs text-slate-300">{formatDateTime(position.lastExchangeSyncTimeMs)}</td>
                </tr>
              ))}
              {positions.length === 0 ? (
                <tr>
