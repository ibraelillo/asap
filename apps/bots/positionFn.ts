...
import {OrquestratorStarted, PositionChanged, PositionClosed,} from "@repo/events";
...
export const onPositionChanged = bus.subscriber(PositionChanged, async (e) => {
    ...
    if (position.isOpen) {
        ...
        console.log({ symbol: position.symbol, multiplier, price: normalizePrice(position.avgEntryPrice, multiplier)})

        // Calculate take profit price based on position side
        // Long: entry + (entry * takeProfitPercent)
        // Short: entry - (entry * takeProfitPercent)
        const tp = takeProfitPrice(position.avgEntryPrice, position.positionSide, 1, multiplier);


        const so = securityOrder(position.avgEntryPrice, position.positionSide, 1.05, multiplier)
...
export const onPositionClosed = bus.subscriber(PositionClosed, async (e) => {
    ...
    const valueQty = /*bot.equity.size ||*/ (await service.accounts.balance()).available / 100 * maxLeverage/*bot.equity.percentage * bot.equity.maxLeverage*/
    ...
    try {
        ...
        console.log(`Reopened position for ${position.symbol} ${position.positionSide}`);
    } catch (error) {
        ...
        const results = await Promise.allSettled(
            e.properties.symbols.map(async (symbol) => {
                ...
                for (const positionSide of ['LONG', 'SHORT']) {
                    try {
                        const {maxLeverage} = await service.market.normalize(/*bot.*/ symbol);
                        ...
                        if (!p) {
                            ...
                            console.log(`Opened position for ${/*bot.*/symbol} ${/*bot.*/positionSide}`);
                        }
                    } catch (error) {
                        console.error(
